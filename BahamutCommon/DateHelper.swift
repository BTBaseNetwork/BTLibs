...
public extension Date {
    ...
    func addMinutes(_ minutes: Int) -> Date {
        return addSeconds(TimeInterval(minutes * 60))
    }

    func addSeconds(_ seconds: TimeInterval) -> Date {
        let copy = Date(timeIntervalSinceNow: timeIntervalSinceNow)
...
public extension Date {
    public func toFriendlyString(_ formatter: DateFormatter! = nil) -> String {
        ...
        if interval < 60 {
            ...
            let mins = String(format: "%.0f", abs(totalMinutesSinceNow.doubleValue))
            return String(format: "X_MINUTES_AGO".bahamutCommonLocalizedString, mins)
        } else if interval < 3600 * 24 {
            let hours = String(format: "%.0f", abs(totalHoursSinceNow.doubleValue))
            return String(format: "X_HOURS_AGO".bahamutCommonLocalizedString, hours)
        } else if interval < 3600 * 24 * 7 {
            let days = String(format: "%.0f", abs(totalDaysSinceNow.doubleValue))
...
extension TimeZone{
    static var isInBeiJingTime:Bool{
        return current.secondsFromGMT() == 28800
    }
}
