...
public class SQLiteTableSet<T>: BTDBTableSet<T> where T: BTDBEntityModel {
    ...
    private func getConstraints(property: BTDBPropertyBase) -> [String] {
        var strs = [String]()
    ...
    private func getColumnType(property: BTDBPropertyBase) -> String {
    ...
    private func getColumnDesc(property: BTDBPropertyBase) -> String {
    ...
    public override func query(sql: String, parameters: [Any]?) -> [T] {
        if let resultSet = try? database.executeQuery(sql, values: parameters) {
            ...
            while resultSet.next() {
                ...
                for property: BTDBProperty<T> in entity.getProperties() {
    ...
    public override func add(model: T) -> T {
        let properties: [BTDBProperty<T>] = entity.getProperties()
    ...
    public override func update(model: T, upsert: Bool) -> T {
        let priProperties: [BTDBProperty<T>] = entity.getPrimaryKey()
        ...
        if let _: T = query(sql: querySql, parameters: priValues).first {
            let notpriProperties: [BTDBProperty<T>] = entity.getNotPrimaryKey()
